...
contract('FundraiserFactory: createFundraiser', (accounts) => {
  ...
  it('increase fundraisersCount', async () => {
...
contract('FundraiserFactory: fundraisers', (accounts) => {
  ...
  describe('varying limits', () => {
    ...
    it('returns maxLimit = 20 results when limit is 30', async () => {
      const fundraisers = await factory.fundraisers(30, 0)

      assert.equal(
        fundraisers.length,
        maxLimit,
        'the number of instances should not exceed maxLimit = 20. 20 instances created!'
      )
    })
  })
  ...
  describe('boundary conditions', () => {
    ...
    it('raise out of bounds error', async () => {
      try {
